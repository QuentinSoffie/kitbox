...
namespace Kitbox.StoreKeeper.Views
{
    ...
    public partial class ViewInfo : UserControl
    {
        ...
        {
            DataBase.Open();
            MySqlDataReader reader = MethodsDB.DataBaseMethods.SqlSearchCustomer(this.Order.CustomerId, DataBase);
            while (reader.Read())
            {
        ...
        private void GetDetailsFromDB(List<string> ListCode)
        {
            ...
            {
                DataBase.Open();
                MySqlDataReader reader = MethodsDB.DataBaseMethods.SqlSearch("Piece", "Code", string.Format("'{0}'", code), DataBase);
                while (reader.Read())
                {
        ...
        public void UpdateCustomer()
        {
            if (pepTextbox1.Text != "")
            {
                MethodsDB.DataBaseMethods.SqlUpdateCustomer("surname", pepTextbox1.Text, int.Parse(Order.CustomerId), DataBase);
                MethodsDB.DataBaseMethods.SqlUpdateCustomerOrder("Customer", pepTextbox1.Text, int.Parse(Order.CustomerId), DataBase);

            }
            if (pepTextbox2.Text != "")
            {
                MethodsDB.DataBaseMethods.SqlUpdateCustomer("firstname", pepTextbox2.Text, int.Parse(Order.CustomerId), DataBase);
            }
            if (pepTextbox3.Text != "")
            {
                MethodsDB.DataBaseMethods.SqlUpdateCustomer("phone", pepTextbox3.Text, int.Parse(Order.CustomerId), DataBase);
            }
            if (pepTextbox4.Text != "")
            {
                MethodsDB.DataBaseMethods.SqlUpdateCustomer("email", pepTextbox4.Text, int.Parse(Order.CustomerId), DataBase);
            }
            if (pepTextbox5.Text != "")
            {
                MethodsDB.DataBaseMethods.SqlUpdateCustomer("address", pepTextbox5.Text, int.Parse(Order.CustomerId), DataBase);
            }
